...
const (
	// DeletionTimeout is the default timeout for waiting for HCP deletion
...
// 4. Delete secrets in HCP namespace
// 5. Wait for HostedCluster deletion (with finalizer removal retry if needed)
func BreakHostedCluster(testCtx *internal.TestContext) error {
	...
	// Step 4: Delete secrets in HCP namespace
	if err := DeleteControlPlaneSecrets(testCtx); err != nil {
		return fmt.Errorf("failed to delete control plane secrets: %w", err)
	}

	// Step 5: Wait for HostedCluster deletion
	...
	if err != nil {
		...
		retryErr := waitForHCDeletion(testCtx, hcName, hcNamespace, DeletionTimeout)
		if retryErr != nil {
...
// DeleteControlPlaneSecrets deletes all secrets in the HCP namespace.
// This is useful for cleanup during test teardown or simulating disaster scenarios.
func DeleteControlPlaneSecrets(testCtx *internal.TestContext) error {
	...
	if err := testCtx.MgmtClient.List(testCtx.Context, secretList, crclient.InNamespace(hcpNamespace)); err != nil {
		return fmt.Errorf("failed to list secrets in namespace %s: %w", hcpNamespace, err)
	}

	for i := range secretList.Items {
		secret := &secretList.Items[i]
		if err := testCtx.MgmtClient.Delete(testCtx.Context, secret); err != nil {
			if !apierrors.IsNotFound(err) {
				return fmt.Errorf("failed to delete secret %s in namespace %s: %w", secret.Name, hcpNamespace, err)
			}
		}
	}
...
func waitForHCPDeletion(testCtx *internal.TestContext, hcpName, hcpNamespace string, timeout time.Duration) error {
	return wait.PollUntilContextTimeout(testCtx.Context, PollInterval, timeout, true, func(ctx context.Context) (bool, error) {
		...
		if err != nil {
			...
			return false, fmt.Errorf("unexpected error checking HCP deletion: %w", err)
		}
...
func waitForHCDeletion(testCtx *internal.TestContext, hcName, hcNamespace string, timeout time.Duration) error {
	return wait.PollUntilContextTimeout(testCtx.Context, PollInterval, timeout, true, func(ctx context.Context) (bool, error) {
		...
		if err != nil {
			...
			return false, fmt.Errorf("unexpected error checking HC deletion: %w", err)
		}
